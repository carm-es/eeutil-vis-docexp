...
public class NewPdfConversion {
	...
	/**
	 * Convierte una lista de documentos PDF a otro documento PDF en el que se
	 * concatenan todos, por orden, y además se reduce el tamanio de las
	 * paginas.
	 * 
	 * @param listaDocs
	 * @param options
	...
	public byte[] copyPdf(File filePdf) throws DocumentException, IOException {
		...
		try {
			...
			for (int i = 0; i < totalPages; i++) {
				...
				copy.addPage(page);
				logger.debug("[copyPdf] Descriptores abiertos despues copy.getImportedPage bucle :" + FdUtils.getFdOpen());
	...
	private byte[] getStamper(File filePdf) throws IOException, DocumentException {
		...
		try {
			...
			logger.debug("[getStamper] Descriptores abiertos despues new PdfStamper bucle :" + FdUtils.getFdOpen());
			...
			logger.debug("[getStamper] Descriptores abiertos despues cerrar stamper :" + FdUtils.getFdOpen());
			
			PdfUtils.close(reader);
			logger.debug("[getStamper] Descriptores abiertos despues cerrar reader :" + FdUtils.getFdOpen());
		}
	...
	private File resizePdf(byte[] file, PdfOptions options) throws IOException,
			DocumentException {
	...
	private Image getImgImportPage(PdfReader reader, PdfWriter writer, int page, Document document) throws BadElementException {
	...
	private Image getImgImportPage(PdfReader reader, PdfWriter writer, int page, Document document ,int orientation) throws BadElementException {
		PdfImportedPage importPage = writer.getImportedPage(reader, page);
    	Image img = Image.getInstance(importPage);
		int rotation = importPage.getRotation();
//		
	...
	private void calculatePercent(float pagePercent, Image img, Document document, int orientation) {
		...
		if (pagePercent == 0) {
			if (img.getScaledWidth() > document.getPageSize().getWidth() 
				...
				//Por defecto dejamos calculo para si la orientacion es vertical (orientation ==PageFormat.PORTRAIT)
				float newPercentWidth = (document.getPageSize().getWidth() - 60)/ img.getWidth();
				float newPercentHeight = (document.getPageSize().getHeight() - 120)/ img.getHeight();
				
				//si la orientacion es horizontal se cambia el calculo
				if (orientation == PageFormat.LANDSCAPE)
				{//no se como calcular esto. le ponemos a 0.57 un tamanio que entra sin ponerse encima de csv
					//newPercentWidth = (document.getPageSize().getHeight() - 60)/ img.getHeight();
					//newPercentHeight = (document.getPageSize().getWidth() - 120)/ img.getWidth();
