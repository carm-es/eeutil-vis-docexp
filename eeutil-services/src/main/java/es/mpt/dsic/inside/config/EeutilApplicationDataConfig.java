...
public class EeutilApplicationDataConfig {
	...
	static {
		if(!CARGADO_PATH) {
			...
			String configPath = System.getProperty(props.getProperty(APPLICATION_NAME_PROPERTY) + "." + CONFIG_PATH_SYSTEM_VAR);
			if (configPath == null) {
				// Si no se ha especificado mediante propiedad de sistema, se busca en el
				// entorno
				configPath = System.getenv(props.getProperty(APPLICATION_NAME_PROPERTY) + "." + CONFIG_PATH_SYSTEM_VAR);
			}
			...
			if (configPath == null) {
				...
				configLog(props);
				logger.debug("Encontrada variable " + props.getProperty(APPLICATION_NAME_PROPERTY) + "." + CONFIG_PATH_SYSTEM_VAR+": " + configPath);
			}
			CARGADO_PATH=true;
		}
	}
	static void configLog(Properties props){
		if(LOAD_LOGGER)
		{
			try {
				...
				File fileLog = new File(CONFIG_PATH + System.getProperty("file.separator") + "log4j-" + props.getProperty(APPLICATION_NAME_PROPERTY) + ".properties");				
				if (fileLog.exists()) {
	...
	static void setConfigPath(String configPath, Properties props) {
		...
		/*
		ApplicationConfig.setApplicationVersion(props
				.getProperty(APPLICATION_VERSION_PROPERTY));
		 */
		if ((APLICACION == null) || APLICACION.equals("sin-definir")) {
			logger.warn("No se encontro la propiedad " + APPLICATION_NAME_PROPERTY
					+ " en el fichero " + APP_PROPERTIES_FILE
					+ "; los ficheros de configuracion se buscaron en el directorio "
					+ CONFIG_PATH);
		}
	}
	...
	public static Properties loadProperties(String file) {
		...
		try {
			...
			logger
			.warn("No se encontro el fichero "
					+ APP_PROPERTIES_FILE
					+ "; los ficheros de configuracion se buscaran solo en el directorio "
					+ CONFIG_PATH);
		}
		return props;
	}
