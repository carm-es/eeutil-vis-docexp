...
public class ConvertDataEeutil {
	...
	public static List<AplicacionObject> converDataEeutil(
			List<EeutilAplicacion> data) {
	...
	public static EeutilAplicacion converDataEeutil(AplicacionObject data)
		...
		if (data.getAditionalData() != null) {
			if (data.getAditionalData().containsKey("firma")
					&& data.getAditionalData().get("firma")
							.equalsIgnoreCase("S")) {
		...
		if (data.getAditionalData() != null) {
			if (data.getAditionalData().containsKey("sello")
					&& data.getAditionalData().get("sello")
							.equalsIgnoreCase("S")) {
		...
		if (data.getAditionalData() != null) {
			if (data.getAditionalData().containsKey("tramitar")
					&& data.getAditionalData().get("tramitar")
							.equalsIgnoreCase("S")) {
		...
		if (data.getAditionalData() != null) {
			retorno.setPropiedades(convertPropiedades(data.getIdentificador(),
		...
		if (data != null) {
			for (String clave : data.keySet()) {
				if (StringUtils.isNotEmpty(clave)
					&& StringUtils.isNotEmpty(data.get(clave))) {
					retorno.add(new EeutilAplicacionPropiedad(
						new EeutilAplicacionPropiedadId(app, clave), data
								.get(clave)));
				}
			}
		}
		...
		if (CollectionUtils.isNotEmpty(data.getPropiedades())) {
			for (EeutilAplicacionPropiedad propiedad : data.getPropiedades()) {
				if (!retorno.containsKey(propiedad.getId().getPropiedad())) {
					retorno.put(propiedad.getId().getPropiedad(),
							propiedad.getValor());
				}
			}
		}
	...
	public static List<AplicacionOperacionObject> converDataEeutilAplicacionPeticion(
		...
		if (CollectionUtils.isNotEmpty(data)) {
			for (EeutilAplicacionOperacion appOperacion : data) {
				retorno.add(new AplicacionOperacionObject(appOperacion
						.getIdAplicacion(), appOperacion.getOperacion(),
						appOperacion.getNumeroPeticiones(), appOperacion
								.isCapturar()));
			}
		}
		return retorno;
	...
	public static List<PeticionObject> converDataPeticion(
			List<EeutilPeticion> data) {
		List<PeticionObject> retorno = new ArrayList<PeticionObject>();
		if (CollectionUtils.isNotEmpty(data)) {
			for (EeutilPeticion peticion : data) {
				PeticionObject obj = new PeticionObject(
