...
public class EeutilAplicacionServiceImpl implements EeutilAplicacionService {
	...
	protected static final Log logger = LogFactory
			.getLog(EeutilAplicacionServiceImpl.class);
		...
		try {
			if (!connectedInside) {
				if (credentialProperties != null) {
					try {
						urlIns = credentialProperties
								.getProperty("inside.admin.url");
						logger.debug(String.format(
								"El WS de Inside se encuentra en %s", urlIns));
						urlInside = new URL(urlIns);
					} catch (MalformedURLException me) {
						logger.error(
								"No se puede crear la URL del servicio admin de Inside "
										+ urlIns, me);
					}
			...
			logger.error("No se puede conectar al servicio Admin de Inside "
					+ urlIns, e);
		}
	...
	public Boolean configureCsvStorage() {
		...
		try {
			if (!connectedCsvStorage) {
				...
				try {
					urlCsvSto = credentialProperties
							.getProperty("csvStorage.admin.url");
					logger.debug(String.format(
							"El WS de CsvStorage se encuentra en %s",
							urlCsvSto));
					urlCsvStorage = new URL(urlCsvSto);
				} catch (MalformedURLException me) {
					logger.error(
							"No se puede crear la URL del servicio Csv Storage "
									+ urlCsvSto, me);
				}
				
				AdminWSService_Service ssCsv = new AdminWSService_Service(
						urlCsvStorage);
			...
			logger.error(
					"No se puede conectar al servicio Admin de Csv Storage "
							+ urlCsvSto, e);
		}
	...
	public Boolean configureCsvBroker() {
		...
		try {
			if (!connectedCsvBroker) {
				...
				try {
					urlCsvBro = credentialProperties
							.getProperty("csvBroker.admin.url");
					logger.debug(String.format(
							"El WS de CsvBroker se encuentra en %s",
							urlCsvBro));
					urlCsvBroker = new URL(urlCsvBro);
				} catch (MalformedURLException me) {
					logger.error(
							"No se puede crear la URL del servicio Csv Broker "
									+ urlCsvBro, me);
				}
							
				es.mpt.dsic.eeutil.csvbroker.admin.consumer.AdminWSService_Service ssCsv = 
						new es.mpt.dsic.eeutil.csvbroker.admin.consumer.AdminWSService_Service(
						urlCsvBroker);
			...
			logger.error(
					"No se puede conectar al servicio Admin de Csv Broker "
							+ urlCsvBro, e);
		}
	...
	private void configureInside(String url) {
		...
		final String passwordCertificado = propertiesSecurity
				.getProperty("passwordCertificado");

		org.apache.cxf.endpoint.Client client = ClientProxy
				.getClient(adminInsideWS);
	...
	private void configureCsvStorage(String url) {
		...
		org.apache.cxf.endpoint.Client client = ClientProxy
				.getClient(adminCsvStorageService);
		org.apache.cxf.endpoint.Endpoint cxfEndpoint = client.getEndpoint();
	...
	public List<AplicacionObject> getAplicaciones(String app)
		...
		if (app.equals("Inside") && configureInside()) {
			retorno = ConvertDataInside.converDataInside(adminInsideWS
					.listaAplicaciones());
		} else if (app.equals("Eeutil")) {
			List<EeutilAplicacion> data = eeutilDao
					.getAllObjetos(EeutilAplicacion.class);
			...
			retorno = ConvertDataCsvStorage.convertData(adminCsvStorageService
					.listaAplicaciones(parameters));
		} else if (app.equals("CsvBroker") && configureCsvBroker()) {
			es.mpt.dsic.eeutil.csvbroker.admin.model.ListaAplicaciones parameters = new es.mpt.dsic.eeutil.csvbroker.admin.model.ListaAplicaciones();
			retorno = ConvertDataCsvBroker.convertData(adminCsvBrokerService
					.listaAplicaciones(parameters));
		}
		return retorno;
	}
	...
	@Override
	public Map<String, String> getInfAdicional(String app)
			throws InsideWSException, CSVStorageException_Exception,
			CSVStorageException, CSVBrokerException {
		...
		if (app.equals("Inside") && configureInside()) {
			...
			retorno = ConvertDataCsvStorage
					.convertInfAdicional(adminCsvStorageService
							.infAdicionalAltaApp(criteria));
		} else if (app.equals("CsvBroker") && configureCsvBroker()) {
			es.mpt.dsic.eeutil.csvbroker.admin.model.InfAdicionalAltaApp criteria = new es.mpt.dsic.eeutil.csvbroker.admin.model.InfAdicionalAltaApp();
			retorno = ConvertDataCsvBroker
					.convertInfAdicional(adminCsvBrokerService
							.infAdicionalAltaApp(criteria));
		}
		return retorno;
	}
	...
	@Override
	public AplicacionObject desactivar(String app,
			AplicacionObject aplicacion) throws InsideWSException,
			CSVStorageException, CSVBrokerException {
		AplicacionObject retorno = null;
		if (app.equals("Inside") && configureInside()) {
			Aplicacion apli = ConvertDataInside
					.converDataFromInside(aplicacion);
			...
			retorno = ConvertDataCsvStorage.convertData(adminCsvStorageService
					.bajaAplicacion(parameter));
		} else if (app.equals("CsvBroker") && configureCsvBroker()) {
			es.mpt.dsic.eeutil.csvbroker.admin.model.BajaAplicacion parameter = new es.mpt.dsic.eeutil.csvbroker.admin.model.BajaAplicacion();
			parameter.setIdAplicacion(aplicacion.getIdentificador());
			retorno = ConvertDataCsvBroker.convertData(adminCsvBrokerService
					.bajaAplicacion(parameter));
		}
	...
	@Override
	public AplicacionObject activar(String app,
			AplicacionObject aplicacion) throws InsideWSException,
			CSVStorageException, CSVBrokerException {
		AplicacionObject retorno = null;
		if (app.equals("Inside") && configureInside()) {
			throw new InsideWSException("No se ha podido activar la aplicación");
		} else if (app.equals("Eeutil")) {
			EeutilAplicacion apli = (EeutilAplicacion) eeutilDao.getObjeto(
					EeutilAplicacion.class, aplicacion.getIdentificador());
			apli.setActiva(true);
			...
			throw new CSVStorageException("No se ha podido activar la aplicación");
		} else if (app.equals("CsvBroker") && configureCsvBroker()) {
			throw new CSVBrokerException("No se ha podido activar la aplicación");
		}
		return retorno;
	}
	
	@Override
	public void eliminarAplicacion(String app,
			AplicacionObject aplicacion, Locale locale) throws InsideWSException,
			CSVStorageException, CSVBrokerException {
		if (app.equals("Inside") && configureInside()) {
			throw new InsideWSException(messageSource.getMessage("listaAplicaciones.eliminar.error", null, locale));
		} else if (app.equals("Eeutil")) {
			removeAppEeutil(aplicacion.getIdentificador());
		} else if (app.equals("CsvStorage") && configureCsvStorage()) {
			throw new CSVStorageException(messageSource.getMessage("listaAplicaciones.eliminar.error", null, locale));
		} else if (app.equals("CsvBroker") && configureCsvBroker()) {
			throw new CSVBrokerException(messageSource.getMessage("listaAplicaciones.eliminar.error", null, locale));
		}
	}

	@Override
	public AplicacionObject altaAplicacion(String app,
			AplicacionObject aplicacion) throws InsideWSException,
			CSVStorageException, CSVBrokerException {
		try {
			AplicacionObject retorno = null;
			if (app.equals("Inside") && configureInside()) {
				...
				EeutilAplicacion apli = ConvertDataEeutil
						.converDataEeutil(aplicacion);
				apli.setActiva(true);
				Object dataBBDD = eeutilDao.getObjeto(EeutilAplicacion.class,
						apli.getIdaplicacion());
				if (dataBBDD != null) {
					if (StringUtils.isEmpty(aplicacion.getPassword())) {
						apli.setPassword(((EeutilAplicacion) dataBBDD)
								.getPassword());
					}
				...
				parameter.setAplicacion(ConvertDataCsvStorage
						.convertData(aplicacion));
				retorno = ConvertDataCsvStorage
						.convertData(adminCsvStorageService
								.altaAplicacion(parameter));
			} else if (app.equals("CsvBroker") && configureCsvBroker()) {
				es.mpt.dsic.eeutil.csvbroker.admin.model.AltaAplicacion parameter = new es.mpt.dsic.eeutil.csvbroker.admin.model.AltaAplicacion();
				parameter.setAplicacion(ConvertDataCsvBroker
						.convertData(aplicacion));
				retorno = ConvertDataCsvBroker
						.convertData(adminCsvBrokerService
								.altaAplicacion(parameter));
			}
			return retorno;
	...
	private void removeAppEeutil(String identificador) {
		EeutilAplicacion appEntity = (EeutilAplicacion) eeutilDao.getObjeto(
				EeutilAplicacion.class, identificador);
		...
		List<PeticionesPDFA> peticionesPdfA = eeutilDao.findObjetos(PeticionesPDFA.class, critPeticionesPdfA);
		...
		List<EeutilAplicacionOperacion> appOperaciones = eeutilDao.findObjetos(EeutilAplicacionOperacion.class, critAppOperacion);
