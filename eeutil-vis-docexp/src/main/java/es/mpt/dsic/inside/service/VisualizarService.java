...
public class VisualizarService {
	...
	protected final static Log logger = LogFactory
			.getLog(VisualizarService.class);
	...
	public SalidaVisualizacion visualizar(ApplicationLogin info, Item item,
		...
		try {
			...
			if ("modelo1".contentEquals(opciones.getModelo())) {
				docSalida = visualizacionExpedienteService.doVisualizable(item,
						opciones);
			} else if ("modelo2".contentEquals(opciones.getModelo())) {
				docSalida = visualizacionDocumentoService.doVisualizable(item,
						opciones);
			} else {
				throw new InSideException(
						"El valor del dato <modelo> es incorrecto",
						new EstadoInfo());
			}
			...
			throw new InSideException("Error al visualizar contenido",
					estadoInfo);
		} catch (Exception e) {
			EstadoInfo estadoInfo = new EstadoInfo();
			logger.error(e);
			throw new InSideException("No se puede obtener el foliado "
					+ e.getMessage(), estadoInfo);
		}
	}

	public SalidaVisualizacion visualizarContenidoOriginal(
			ApplicationLogin info, Item item) throws InSideException {
		try {
			DocumentoContenido docSalida = visualizacionDocumentoService
					.doVisualizableOriginal(item);
	...
	public SalidaVisualizacion visualizarDocumentoConPlantilla(
			ApplicationLogin info,
			DocumentoEniConMAdicionales docEniAdicionales, String plantilla)
			throws InSideException {
		try {
			byte[] plantillaBytes = getPlantilla(info.getIdApplicacion(),
					plantilla);

			es.mpt.dsic.inside.fop.model.documento.DocumentoEniConMAdicionales modelObject = documentConverter
					.documentoEniConMAdicionalesWsToToModel(docEniAdicionales,
							afirmaService, info.getIdApplicacion());

			DocumentoContenido docSalida = fopConverter.convertDocumentToPdf(
					modelObject, plantillaBytes);
	...
	private byte[] getPlantilla(String idAplicacion, String idPlantilla)
			throws InSideException {
		AppInfo aplicacion = aplicacionInfoService
				.getAplicacionInfo(idAplicacion);
		byte[] plantilla = aplicacion.getPlantilla(idPlantilla);
		if (plantilla == null) {
			throw new InSideException(new EstadoInfo(
					"Error al visualizar contenido. Plantilla no válida", null,
					"La aplicación " + idAplicacion
							+ " no tiene asociada la plantilla " + idPlantilla));
		} else {
			return plantilla;
		}
	}
	...
	public List<Plantilla> obtenerPlantillas(ApplicationLogin info)
			throws InSideException {
		AppInfo aplicacion = aplicacionInfoService.getAplicacionInfo(info
				.getIdApplicacion());
	...
	public List<Plantilla> asociarPlantilla(ApplicationLogin info,
			String idPlantilla, byte[] plantilla) throws InSideException {
		AppInfo aplicacion = aplicacionInfoService.asociarPantilla(
				info.getIdApplicacion(), idPlantilla, plantilla);
		return convertPlantillaData(aplicacion.getPlantillas());
	}

	public List<Plantilla> eliminarPlantilla(ApplicationLogin info,
			String idPlantilla) throws InSideException {
		AppInfo aplicacion = aplicacionInfoService.eliminarPlantilla(
				info.getIdApplicacion(), idPlantilla);
		return convertPlantillaData(aplicacion.getPlantillas());
	}
