...
protected final static Log logger = LogFactory.getLog(CabeceraAdder.class);
	...
	//public byte[] addCabecera (byte[] pdfEntrada, String rutaLogo, String nombreExpediente, String[] lineasOrganismo) throws ContentNotAddedException {
	//public byte[] addCabecera (byte[] pdfEntrada, String rutaLogo, String nombreEntidad, String valorEntidad, String nombreDocumento, String[] lineasOrganismo) throws ContentNotAddedException {
		public byte[] addCabecera (byte[] pdfEntrada, String rutaLogo, String descripcion1, String descripcion2, String[] lineasOrganismo) throws ContentNotAddedException {
		...
		byte[] bytesSalida = null;
			...
			try  {
				...
				PdfPTableDesigner designerHorizontal = new CabeceraVisualizacionIndiceDesigner(rutaLogo, widthTableHor, relativeWidthsHor);
				PdfPTableDesigner designerVertical = new CabeceraVisualizacionIndiceDesigner(rutaLogo, widthTableVer, relativeWidthsVer);
								
				for (int i=1; i<=n; i++) {
					
					//PdfPTableDesigner des = new CabeceraVisualizacionIndiceDesigner(rutaLogo, widthTable, relativeWidths);
					
					//String[] lineasDerecha = {"P�gina " + i, "Expediente Electr�nico: " + nombreExpediente,  "Documento �ndice"};
					String[] lineasDerecha = {tPagina + " " + i, descripcion1, descripcion2};
					Object[] params = {rutaLogo, lineasOrganismo, lineasDerecha};
					PdfPTable cabecera;
					// Vertical
					if (stamp.getImportedPage(reader, i).getHeight() > stamp.getImportedPage(reader, i).getWidth()) {
					...
					cabecera.writeSelectedRows(0, -1, tablaSepEjeX, stamp.getImportedPage(reader, i).getHeight() - tablaSepEjeY, stamp.getOverContent(i));
				}
