...
public class TemplateConverter {
	 ...
	 */
	public DocumentoContenido convertDocumentToPdf(
			DocumentoEniConMAdicionales docEniAdicionales, byte[] jrxml)
			throws Exception {
    	...
    	byte[] contenidoConvertido = convertContent(docEniAdicionales.getDocumento().getContenido().getValorBinario());
	...
	private byte[] convertContent(byte[] contenido) {
		...
		try {
			...
			String ipOO = aplicacionContext.getAplicacionInfo()
					.getPropiedades().get("ip.openoffice");
			String portOO = aplicacionContext
