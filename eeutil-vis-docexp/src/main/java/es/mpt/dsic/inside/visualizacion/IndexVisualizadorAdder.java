...
public class IndexVisualizadorAdder {
	...
	public void addContent(List<VisualizacionItem> listaItems, Document doc,
			PdfWriter pw) throws ContentNotAddedException {
	 ...
	 */
	protected void printIndex (List<VisualizacionItem> listaItems, Document doc, PdfWriter pw) throws DocumentException{
		...
		for (int i=0; i < listaItems.size(); i++) {
			...
			/*************** TAMAÑO DE FUENTE DEPENDIENDO DE LA PROFUNDIDAD Y COLOR DE LAS FILAS INTERCALADO
			// El color de fila
			BaseColor colorCell = backgroundRows [i % backgroundRows.length] ;
			
			// Fuente de la lÃ­nea, mÃ¡s pequeÃ±a cuanto mayor sea la profundidad del elemento en el Ã­ndice (subapartados letra mÃ¡s pequeÃ±a).
			Font font = new Font (fontBase.getFamily(), 
								  fontBase.getSize() - (item.getProfundidadEnIndice() + 2));
			
			PdfPTable fila = fila (item, offset, font, colorCell, doc, pw);
			*****************FIN TAMAÑO DE FUENTE DEPENDIENDO DE LA PROFUNDIDAD Y COLOR DE LAS FILAS INTERCALADO**********************/
	 ...
	 */
	protected PdfPTable tablaItem (final VisualizacionItem item, final Font font, final BaseColor bgColor) throws DocumentException{
		PdfPTable table = new PdfPTable(1);
		table.setWidthPercentage(widthPercentage);
	 ...
	 */
	protected PdfPTable tablaPropiedades (final VisualizacionItem item, final Font font, final BaseColor bgColor) throws DocumentException {
