...
public class DocumentConverter {
	...
	public DocumentoEniConMAdicionales documentoEniConMAdicionalesWsToToModel(
			es.mpt.dsic.inside.ws.service.model.documento.DocumentoEniConMAdicionales input, AfirmaService afirmaService, String aplicacion) {
		...
		retorno.setMetadatoAdicional(metadatoAdicionaWsToModel(input
				.getMetadatosAdicionales()));
		return retorno;
	}

	private List<MetadatoAdicional> metadatoAdicionaWsToModel(
			TipoMetadatosAdicionales input) {
		List<MetadatoAdicional> retorno = new ArrayList<MetadatoAdicional>();
		if (input != null
				&& CollectionUtils.isNotEmpty(input.getMetadatoAdicional())) {
	...
	private TipoDocumento tipoDocumentoWsToModel(
			es.mpt.dsic.inside.ws.service.model.eni.documento.TipoDocumento input, AfirmaService afirmaService, String aplicacion) {
	...
	private List<TipoFirmasElectronicas> firmasWsToModel(
			List<es.mpt.dsic.inside.ws.service.model.eni.firma.TipoFirmasElectronicas> input, AfirmaService afirmaService, String aplicacion) {
					...
					if (afirmaService != null) {
						try {
							InformacionFirmaAfirma infoAfirma = afirmaService
									.obtenerInformacionFirma(aplicacion, firma
											.getContenidoFirma()
											.getFirmaConCertificado()
											.getFirmaBase64(), true, false, true,
											null);
							TipoFirmasElectronicas data = new TipoFirmasElectronicas();
							data.setFechaFirma(infoAfirma.getFirmantes().get(0)
									.getFecha());
							data.setTipoFirma(firma.getTipoFirma().value() + " - " + infoAfirma.getTipoDeFirma()
									.getTipoFirma());
							data.setFirmante(infoAfirma.getFirmantes().get(0)
									.getNombre()
									+ " "
									+ infoAfirma.getFirmantes().get(0)
											.getApellido1()
									+ " "
									+ infoAfirma.getFirmantes().get(0)
											.getApellido2());
	...
	private TipoMetadatos tipoMetadatosWsToModel(
		...
		retorno.setEstadoElaboracion(estadoElaboracionWsToString(input
				.getEstadoElaboracion()));
		retorno.setFechaCaptura(fechaWsToModel(input.getFechaCaptura()));
		retorno.setIdentificador(input.getIdentificador());
		retorno.setOrigenCiudadanoAdministracion(converOrigenCiudadano(input
				.isOrigenCiudadanoAdministracion()));
		retorno.setTipoDocumental(tipoDocumentoWsToModel(input
				.getTipoDocumental()));
	...
	private String tipoDocumentoWsToModel(TipoDocumental input) {
		...
		if (input.equals(TipoDocumental.TD_18)) {
			retorno = "Comunicación ciudadano";
		}
	...
	private String converOrigenCiudadano(boolean input) {
		return input ? "Ciudadano" : "Administración";
	}
	...
	private String estadoElaboracionWsToString(TipoEstadoElaboracion input) {
		String retorno = null;
		if (input.getValorEstadoElaboracion().equals(
				EnumeracionEstadoElaboracion.EE_01)) {
			retorno = "Original";
		}
		if (input.getValorEstadoElaboracion().equals(
				EnumeracionEstadoElaboracion.EE_02)) {
			retorno = "Copia electrónica auténtica con cambio de formato";
		}
		if (input.getValorEstadoElaboracion().equals(
				EnumeracionEstadoElaboracion.EE_03)) {
			retorno = "Copia electrónica auténtica de documento papel";
		}
		if (input.getValorEstadoElaboracion().equals(
				EnumeracionEstadoElaboracion.EE_04)) {
			retorno = "Copia electrónica parcial auténtica";
		}
		if (input.getValorEstadoElaboracion().equals(
				EnumeracionEstadoElaboracion.EE_99)) {
			retorno = "Otros";
		}
		return retorno;
	}
