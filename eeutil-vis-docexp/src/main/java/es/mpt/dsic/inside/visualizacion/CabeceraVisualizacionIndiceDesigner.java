...
public class CabeceraVisualizacionIndiceDesigner implements PdfPTableDesigner {
	...
	public CabeceraVisualizacionIndiceDesigner (String rutaLogo, float widthTabla, float[] relativeWidths){
	...
	@Override
	/**
	 * params[0] = ruta del logo
	 * params[1] = String[] con l�neas del nombre del organismo
	 * params[2] = String[] l�neas de la derecha	 
	 */
	public PdfPTable designTable(Object[] params) throws ContentNotAddedException {
		if (params.length != 3) {
			throw new ContentNotAddedException (new IllegalArgumentException ("El n�mero de argumentos tiene que ser 3:"));
		}
		if (!(params[0] instanceof String)) {
			throw new ContentNotAddedException (new IllegalArgumentException ("El primer argumento debe ser un String"));
		}
		if (!(params[1] instanceof String[])) {
			throw new ContentNotAddedException (new IllegalArgumentException ("El segundo argumento debe ser un array de Strings"));
		}
		if (!(params[2] instanceof String[])) {
			throw new ContentNotAddedException (new IllegalArgumentException ("El tercer argumento debe ser un array de Strings"));
		}
		...
		try {
			...
			throw new ContentNotAddedException ("No se puede obtener la imagen a partir de: " + (String) params[0], e);
		}
		...
		tabla.addCell(celdaTablaDerecha (Arrays.copyOfRange(lineasDerecha, 0, lineasDerecha.length - 1)));		
		
		// La �ltima cadena se pinta en una nueva celda (con colspan) de la tabla general, para conseguir que se pinte la l�nea recta.		
		tabla.addCell(celdaAbajo(lineasDerecha[lineasDerecha.length-1], 0.5f));
		
		
		// Todas las cadenas menos la �ltima se escribir�n en una tabla, a la derecha.
		//tabla.addCell(celdaTablaDerecha (Arrays.copyOfRange(lineasDerecha, 0, lineasDerecha.length - 1)));
		//tabla.addCell(celdaTablaDerecha (Arrays.copyOfRange(lineasDerecha, 0, lineasDerecha.length - 2)));
