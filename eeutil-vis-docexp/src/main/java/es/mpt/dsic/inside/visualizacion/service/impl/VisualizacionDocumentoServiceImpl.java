...
public class VisualizacionDocumentoServiceImpl implements VisualizacionService {
	...
	protected final static Log logger = LogFactory
			.getLog(VisualizacionDocumentoServiceImpl.class);
	...
	@Override
	public DocumentoContenido doVisualizable(Item item,
			OpcionesVisualizacion oVisualizacion) throws VisualizacionException {
		...
		try {
			...
			List<VisualizacionItem> visItems = VisualizacionUtils
					.obtenerVisualizacionItems(item);

			indexVisualizadorAdder.setTitle(tDocElectronico.toUpperCase());
			indexVisualizadorAdder.setNameColumnsContent(new String[] {
					tTipoFirma.toUpperCase(),
					tFirmante.toUpperCase() + " / " + tValorCsv.toUpperCase(),
					tFechaFirma.toUpperCase() + " / "
							+ tRegulacionCsv.toUpperCase() });
			indexVisualizadorAdder.setRelativeWidthsContent(new float[] {
					0.30f, 0.35f, 0.35f });
			...
			contentDocumentAdder.setIpOO(aplicacionContext.getAplicacionInfo()
					.getPropiedades().get("ip.openoffice"));
			contentDocumentAdder.setPortOO(aplicacionContext
					.getAplicacionInfo().getPropiedades()
					.get("port.openoffice"));
			contentDocumentAdder.addContent(item.getDocumentoContenido()
					.getBytesDocumento(), doc, pw);
			...
			if (rutaLogo != null
					&& oVisualizacion.getOpcionesLogo() != null
					&& oVisualizacion.getOpcionesLogo().isEstamparLogo()
					&& oVisualizacion.getOpcionesLogo()
							.isEstamparNombreOrganismo()
					&& oVisualizacion.getOpcionesLogo()
							.getListaCadenasNombreOrganismo() != null) {
				...
				pdfResult = cabeceraAdder.addCabecera(pdfResult, rutaLogo,
						tVisualizacionDocElectronico, tDocElectronico + ": "
								+ visItems.get(0).getNombre(),
						(String[]) oVisualizacion.getOpcionesLogo()
								.getListaCadenasNombreOrganismo().getCadenas()
								.toArray(new String[] {}));
			}
			...
			if (oVisualizacion.getOpcionesLogo() != null
					...
					&& !oVisualizacion.getOpcionesLogo().getTextoPie()
							.contentEquals("")) {

				pieAdder.setTablaSepEjeX(doc.leftMargin());
				pieAdder.setTablaSepEjeY(doc.bottomMargin());

				pdfResult = pieAdder.addPie(pdfResult, oVisualizacion
						.getOpcionesLogo().getTextoPie());
	...
	public void setIndexVisualizadorAdder(
			IndexStandardVisualizadorAdder indexVisualizadorAdder) {
	...
	public void setContentDocumentAdder(
			ContentDocumentAdder contentDocumentAdder) {
	...
	public void settVisualizacionDocElectronico(
			String tVisualizacionDocElectronico) {
		this.tVisualizacionDocElectronico = tVisualizacionDocElectronico;
	}

	@Override
	public DocumentoContenido doVisualizableOriginal(Item item)
