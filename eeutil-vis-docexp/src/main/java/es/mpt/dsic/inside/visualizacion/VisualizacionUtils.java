...
public class VisualizacionUtils {
	...
	/**
	 * Recorre la estructura de árbol en preorden para obtener una lista ordenada de Items de Visualizacion.	 * 
	...
	public static  List<VisualizacionItem> obtenerVisualizacionItems (Item nodo) throws Exception{
		...
		/*for (FoliadoItem f : foliadoItems) {
	 ...
	 * @param lista
	 */
	public static void addVisualizacionItemToList (final Item nodo, 
											 int prof,												 
											 List<VisualizacionItem> lista,
											 int[] marcadores) throws Exception{
		...
		if (nodo.getHijos() != null) {
			...
			for (Item hijo : nodo.getHijos().getItems()) {
				...
				nuevosMarcadores[nuevosMarcadores.length - 1] = nodo.getHijos().getItems().indexOf(hijo) + 1;
	...
	public static List<Entry<String, String>> listaPropiedadesToEntryList (final Item nodo) {
		...
		if (nodo.getPropiedades() != null) {
			...
			for (Propiedad propiedad : nodo.getPropiedades().getPropiedades()) {
				Entry<String, String> entry = new AbstractMap.SimpleEntry<String, String>(propiedad.getClave(), propiedad.getValor());
				entryList.add(entry);
			}
		}
