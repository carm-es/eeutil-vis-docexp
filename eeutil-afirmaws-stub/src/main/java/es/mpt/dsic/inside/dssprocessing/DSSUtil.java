...
public class DSSUtil {
	...
	/**
	 * Busca en el VerificationReport de la respuesta el ReadableCertificateInfo que coincida en número de serie con el X509Certificate pasado como parámetro 
	 * @param certificate certificado X509Certificate del que se quiere encontrar su equivalente en el VerificationReport
	 * @param verificationReport
	 * @return ReadableCertificateInfo que tenga el mismo número de serie que el X509, null si no se encuentra ninguno.
	 */
	/*public static ReadableCertificateInfo getReadableCertificateInfoCoincidente (X509Certificate certificate, List<ReadableCertificateInfo> readableCertificateInfoList) {
		//List<IndividualSignatureReportType> listaReports = verificationReport.getIndividualSignatureReport();
		
		int i=0; 
		boolean found = false;
		ReadableCertificateInfo coincidente = null;
		
		while (i < readableCertificateInfoList.size() && !found) {
			//IndividualSignatureReportType report = listaReports.get(i);
			ReadableCertificateInfo certInfo = readableCertificateInfoList.get(i);
			
			String serialNumber = getFieldValue ("numeroSerie", certInfo);
			String idEmisor = getFieldValue("idEmisor", certInfo);
			...
			if (certificate.getSerialNumber().toString().contentEquals(serialNumber)) {
				...
				if (idEmisor != null) {
					if (idEmisor.contentEquals(certificate.getIssuerX500Principal().getName()) ||
							idEmisor.contentEquals(MiscUtil.getOrganizationIssuerX500Principal(certificate.getIssuerX500Principal().getName()))) {
	...
	/**
	 * Devuelve la pareja ReadableCertificateInfo/TstContentType cuyo objeto readableCertificateInfo tenga el mismo número de serie que el X509Certificate
	 * pasado como parámetro.
	 * @param certificate
	 * @param readableCertificateInfoAndTstContentList
	 * @return pareja ReadableCertificateInfo/TstContentType que tenga el mismo número de serie que el X509, null si no se encuentra ninguno.
	 */	
	public static ReadableCertificateInfoAndTstContentType getReadableCertificateInfoAndTstContentTypeCoincidente (X509Certificate certificate,
		...
		while (i < readableCertificateInfoAndTstContentList.size() && !found) {
			//IndividualSignatureReportType report = listaReports.get(i);
			ReadableCertificateInfoAndTstContentType certInfoAndTstContent = readableCertificateInfoAndTstContentList.get(i);
			
			String serialNumber = getFieldValue ("numeroSerie", certInfoAndTstContent.getReadableCertificateInfo());
			String idEmisor = getFieldValue("idEmisor", certInfoAndTstContent.getReadableCertificateInfo());
	 ...
	 * @return
	 */
	/*public static Object getObjectByClass (Class<?> clase, List<Object> lista) {
		int i=0;
		boolean found = false;
		Object ret = null;
		while (i < lista.size() && !found) {
			if (lista.get(i).getClass().equals(clase)) {
				ret = lista.get(i);
				found = true;				
			}
			i++;
	 ...
	 * @param readableCertificateInfoList Información de los certificados firmantes obtenida de la llamada a los servicios DSS de Afirma.
	 * @return Lista de firmantes con la información de ambas fuentes.
	 */
	/*public static List<FirmaInfo> mergeInfoFirmantes (List<InformacionFirmante> infoFirmantes, List<ReadableCertificateInfo> readableCertificateInfoList) {
		...
		for (InformacionFirmante info1 : infoFirmantes) {
			...
			fillFirmaInfo(firmaInfo, getReadableCertificateInfoCoincidente (info1.getCertificado(), readableCertificateInfoList));
	 ...
	 * Mezcla la información de dos listas de firmantes.	 * 
	 * @param infoFirmantes Información de los firmantes obtenida con las librerías de afirma.
	 * @param readableCertificateInfoAndTstContentList Información de los certificados firmantes obtenida de la llamada a los servicios DSS de Afirma, junto con los sellos de tiempo.
	 * @return Lista de firmantes con la información de ambas fuentes.
	 */
	/*public static  List<FirmaInfo> mergeInfoFirmantes (List<InformacionFirmante> infoFirmantes, List<ReadableCertificateInfoAndTstContentType> readableCertificateInfoAndTstContentList) {
		...
		for (InformacionFirmante info1 : infoFirmantes) {
			...
			//fillFirmaInfo(firmaInfo, getReadableCertificateInfoCoincidente (info1.getCertificado(), readableCertificateInfoAndTstContentList));
			ReadableCertificateInfoAndTstContentType certTstInfo = getReadableCertificateInfoAndTstContentTypeCoincidente (info1.getCertificado(), readableCertificateInfoAndTstContentList);
			fillFirmaInfo(firmaInfo, certTstInfo.getReadableCertificateInfo(), certTstInfo.getTimeStampContent());
		...
		if (firmantes != null) {
			//firmantes.get(0).setExtras((InformacionFirmante.TipoFirma.F).toString());
			firmantes.get(0).setExtras(firmantes.get(0).getExtras().replaceFirst((InformacionFirmante.TipoFirma.CF).toString(),
	 ...
	 * @return Lista de firmantes con la información de ambas fuentes.
	 */
	public static  List<FirmaInfoAfirma> mergeInfoFirmantes (List<InformacionFirmante> infoFirmantes, List<ReadableCertificateInfoAndTstContentType> readableCertificateInfoAndTstContentList) {
		...
		for (InformacionFirmante info1 : infoFirmantes) {
			...
			//fillFirmaInfo(firmaInfo, getReadableCertificateInfoCoincidente (info1.getCertificado(), readableCertificateInfoAndTstContentList));
			ReadableCertificateInfoAndTstContentType certTstInfo = getReadableCertificateInfoAndTstContentTypeCoincidente (info1.getCertificado(), readableCertificateInfoAndTstContentList);
			fillFirmaInfoAfirma(firmaInfo, certTstInfo.getReadableCertificateInfo(), certTstInfo.getTimeStampContent());
	...
	/*public static void fillFirmaInfo (FirmaInfo firmaInfo, final ReadableCertificateInfo certInfo) {
		...
		// apellido1 y apellido2 son required, así que les ponemos valor cadena vacía, por si no estuvieran en la información del certificado firmante.
	 ...
	 * @param certInfo Objeto devuelto en la petición al servico DSS de afirma.
	 */
	/*public static void fillFirmaInfo (FirmaInfo firmaInfo, final ReadableCertificateInfo certInfo, final TstContentType tstContent) {
		String nifResponsable = null;
		String NIFCIF = null;
		...
		if (tstContent != null) {
			if (tstContent.getCreationTime() != null) {
				...
				firmaInfo.setExtras(firmaInfo.getExtras() + " - (Sello de Tiempo: " + MiscUtil.dateToString(greg.toGregorianCalendar().getTime(), "dd/MM/yyyy HH:mm") + ")");
			}
	 ...
	 * @param certInfo Objeto devuelto en la petición al servico DSS de afirma.
	 */
	public static void fillFirmaInfoAfirma (FirmaInfoAfirma firmaInfo, final ReadableCertificateInfo certInfo, final TstContentType tstContent) {
		String nifResponsable = null;
		String NIFCIF = null;
		String NIFEntidadSuscriptora = null;
		String entidadSuscriptora = null;
		...
		//aniade tratar posibilidad seudonimo. Si existe seudonimo, es el prioritario en informar dejando los demas fuera
		...
		// los metemos en el primer apellido, para evitar problemas con los apellidos compuestos E.G. "De la Paz" 
	...
	private static boolean siApellidosJuntos(FirmaInfoAfirma firmaInfo, String apellidos) {
		return "".equals(firmaInfo.getApellido1()) && "".equals(firmaInfo.getApellido2()) && apellidos != null;
	}
	 ...
	 * @return
	 */
	/*public static List<ReadableCertificateInfo> getReadableCertificateInfoList (ResponseBaseType verifyResponse) {
		
		List<ReadableCertificateInfo> readableCertificateInfoList = new ArrayList<ReadableCertificateInfo> ();
		
		// Obtenemos el VerificationReport del OptionalOutputs
		List<Object> optionalOutputs = verifyResponse.getOptionalOutputs().getAny();
		
		VerificationReportType vrt = (VerificationReportType) DSSUtil.getObjectByClass(VerificationReportType.class, optionalOutputs);
		...
		for (IndividualSignatureReportType report : indReportList) {
			...
			ReadableCertificateInfo certInfo = (ReadableCertificateInfo) DSSUtil.getObjectByClass(ReadableCertificateInfo.class, details); 
			if (certInfo != null) {
	...
	/**
	 * Obtiene la lista de ReadableCertificateInfo y de TstContent de una respuesta DSS de verificación de firma.
	 * @param verifyResponse
	 * @return
	 */
	public static List<ReadableCertificateInfoAndTstContentType> getReadableCertificateInfoAndTimeStampContentList (ResponseBaseType verifyResponse) {
		
		List<ReadableCertificateInfoAndTstContentType> readableCertificateInfoAndTimeStampContentList = new ArrayList<ReadableCertificateInfoAndTstContentType> ();
		...
		for (IndividualSignatureReportType report : indReportList) {
			ReadableCertificateInfoAndTstContentType certInfoAndTst = new ReadableCertificateInfoAndTstContentType ();
			...
			DetailedReportType dtr = (DetailedReportType) DSSUtil.getObjectByClass(DetailedReportType.class, details);
	...
	public static SignedDataRefType getSignedDataRef (SignedDataInfo signedDataInfo, String atributoIDNodoFirmado) {
		...
		{				
			if (signedDataRefList.get(i).getXPath()!=null && signedDataRefList.get(i).getXPath().contains(atributoIDNodoFirmado)) {
