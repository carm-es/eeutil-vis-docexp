...
public class XMLDetachedDSSSignerProcessor extends XMLDSSSignerProcessor {
	...
	@Override
	public ContenidoFirmado getSignedData(ResponseBaseType verifyResponse,
			byte[] sign) throws ContentNotExtractedException {
		...
		SignedDataRefType signedDataRefConEncodingOrFirst = DSSUtil.getSignedDataRefConEncodingOrFirst (signedDataInfo);
		...
		try {
			...
			String encoding = getEncodingDatosFirmados (nodoContenido, signedDataRefConEncodingOrFirst, doc.getInputEncoding());
	...
	private String getEncodingDatosFirmados (Node nodoContenido, SignedDataRefType signedDataRef, String encod) {
		...
		if (encoding == null) {
			logger.info("No se ha podido obtener el Encoding ni del nodo contenido ni del nodo signedDataRef");
			encoding = encod;
		}
		return encoding;
	}
