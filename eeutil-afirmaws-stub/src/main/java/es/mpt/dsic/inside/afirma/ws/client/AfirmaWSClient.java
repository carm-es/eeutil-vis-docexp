...
public class AfirmaWSClient {
	...
	public ResultadoValidacionInfoAfirma validarFirma(
		...
		//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
		//
		//	COMENTO ESTA LOCURA DE CODIGO PORQUE NO TIENE SENTIDO!!!! SI MANDO A AFIRMA UNA FIRMA PARA VALIDARLA Y YA ME RESPONDE QUE ES
		//	VALIDA/O INVALIDA... A QUE MIRO ESTADO DE LOS CERTIFICADOS!!!!!!! AFIRMA YA HABRA HECHO TODAS LAS COMPROBACIONES NECESARIAS 
...
//////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////////
	...
	public ResultadoValidarCertificadoAfirma validarCertificado(
		...
		// Validacion de certificado. Si no ha habido error en afirma no devuelve campo exception relleno
		if (valCert.getExcepcion()==null) 
		{
			...
			detalleValidacion += construyeDetalleRespuesta(valCert.getResultadoProcesamiento().getResultadoValidacion().getDescripcion());
		}
		else
		{
		...
		if (idUsuario == null) {
			validado = false;
			detalleValidacion = "El NIF del responsable no se encuentra en la informaci�n del certificado";
		}
	...
	public ResponseBaseType ampliarFirmaDSS(
		...
		try {
			...
			VerifyRequest vrequest = requestAmpliarFirmaDSS.getProcessor()
					.buildVerifyRequestToUpgrade(
							requestAmpliarFirmaDSS.getIdAplicacion(),
							requestAmpliarFirmaDSS.getSign(),
			...
			vresponse = (ResponseBaseType) ((JAXBElement<?>) marshallerdssverify
					.unmarshal(new StreamSource(new StringReader(sResult))))
					.getValue();

		} catch (XmlMappingException e) {
			logger.error(
					"Error al hacer marshall o unmarshall" + e.getMessage(), e);
	...
	private String construyeDetalleRespuesta(Object obj) {
		...
		if (obj instanceof afirmaws.services.nativos.model.validarcertificado.MensajeSalida.Respuesta.Excepcion) {
			afirmaws.services.nativos.model.validarcertificado.MensajeSalida.Respuesta.Excepcion e = (afirmaws.services.nativos.model.validarcertificado.MensajeSalida.Respuesta.Excepcion) obj;
			mensaje.append("CodigoError: " + e.getCodigoError() + ln);
			mensaje.append("Descripcion: " + e.getDescripcion() + ln);
			mensaje.append("ExcepcionAsociada: " + e.getExcepcionAsociada()
					+ ln);
			...
			mensaje.append("Resultado: "
					+ res.getResultadoValidacion().getResultado() + ln);
			mensaje.append("Descripcion: "
					+ res.getResultadoValidacion().getDescripcion() + ln);
	...
	private String crearPeticionValidar(RequestValidarFirma requestValidarFirma) {
		...
		parametros.setFirmaElectronica(requestValidarFirma
				.getFirmaElectronica());
	...
	private Respuesta crearPeticionValidarCertificado(
		...
		System.setProperty("javax.net.ssl.trustStore",
				requestValidarCertificado.getTruststore());
		System.setProperty("javax.net.ssl.trustStorePassword",
				requestValidarCertificado.getPassTruststore());

		afirmaws.services.nativos.model.validarcertificado.MensajeEntrada entrada = new afirmaws.services.nativos.model.validarcertificado.MensajeEntrada();
		entrada.setPeticion(AfirmaConstantes.OP_VALIDAR_CERTIFICADO);
		entrada.setVersionMsg(AfirmaConstantes.VERSION_1_0);

		afirmaws.services.nativos.model.validarcertificado.MensajeEntrada.Parametros parametros = new afirmaws.services.nativos.model.validarcertificado.MensajeEntrada.Parametros();

		parametros.setIdAplicacion(requestValidarCertificado.getIdAplicacion());
		parametros.setModoValidacion(requestValidarCertificado
				.getModoValidacion());
		...
		afirmaws.services.nativos.model.validarcertificado.MensajeSalida salidaCertM = getSalidaCert(salidaCert);
	...
	private ResponseBaseType validarFirmaDSS(
		...
		System.setProperty("javax.net.ssl.trustStore",
				requestValidarFirmaDSS.getTruststore());
		System.setProperty("javax.net.ssl.trustStorePassword",
		...
		try {
			...
			VerifyRequest vrequest = requestValidarFirmaDSS.getProcessor()
					.buildVerifyRequest(requestValidarFirmaDSS.getSign(),
							requestValidarFirmaDSS.getIdAplicacion(),
							requestValidarFirmaDSS.getContent());
	...
	public InformacionFirmaAfirma obtenerInformacionFirma(
		...
		try {
			...
			// WS de Afirma.
			DSSSignerProcessor processor = dssProcessorFactory
					.getDSSSignerProcessor(requestObtenerInformacionFirma
							.getFirma());
			if (processor == null) {
				throw new AfirmaException(CodigosError.COD_0001,
						CodigosError.MSJ_0001);
			} 
...
//			}
			...
			RequestValidarFirmaDSS requestValidarFirmaDSS = new RequestValidarFirmaDSS(
					new RequestConfigAfirma(requestObtenerInformacionFirma
							.getIdAplicacion(), requestObtenerInformacionFirma
							.getTruststore(), requestObtenerInformacionFirma
							.getPassTruststore()),
			...
			if (verifyResponse
					...
					&& verifyResponse
							...
							.getResultMinor()
							.contentEquals(
									DSSResultConstantes.DSS_MINOR_INCORRECTFORMAT)) {
				logger.info("Los servicios de Afirma responden IncorrectFormat");
				throw new AfirmaException(CodigosError.COD_0003,
						CodigosError.MSJ_0003);
				...
				// "Firma válida, firma no válida, y warning. Para el resto lanzamos una excepción"
					...
					&& !verifyResponse
							...
							.getResultMajor()
							.contentEquals(
									DSSResultConstantes.DSS_MAJOR_INVALIDSIGNATURE)
					&& !verifyResponse
							.getResult()
							.getResultMajor()
							.contentEquals(
									DSSResultConstantes.DSS_MAJOR_WARNING)) {
			...
			if (requestObtenerInformacionFirma.isObtenerFirmantes()) {
				...
				// librerías de Afirma.
				AOSigner signer = AOSignerFactory
						.getSigner(requestObtenerInformacionFirma.getFirma());
				if (signer == null) {
					throw new AfirmaException(CodigosError.COD_0001,
							CodigosError.MSJ_0001);
				}
				List<InformacionFirmante> firmantes = AOUtilExt
						.getTreeAsInformacionFirmantes(signer
								.getSignersStructure(
										requestObtenerInformacionFirma
												.getFirma(), true));
			...
			if (requestObtenerInformacionFirma.isObtenerDatosFirmados()) {
				if (requestObtenerInformacionFirma.getContent() == null) {
					...
					ContenidoFirmado contenidoFirmado = processor
							.getSignedData(verifyResponse,
									requestObtenerInformacionFirma.getFirma());

					if (contenidoFirmado.getBytesDocumento() != null) {
						ContenidoInfoAfirma documentoFirmado = new ContenidoInfoAfirma();
						documentoFirmado.setContenido(contenidoFirmado
								.getBytesDocumento());
						documentoFirmado.setTipoMIME(contenidoFirmado
								.getMimeDocumento());
						infoFirma.setDocumentoFirmado(documentoFirmado);
					} else if (contenidoFirmado.getHash() != null) {
						infoFirma.setHashFirmado(new String(contenidoFirmado
								.getHash()));
						infoFirma.setAlgoritmoHashFirmado(contenidoFirmado
								.getAlgoritmoHash());
					}
				} else {
					ContenidoInfoAfirma documentoFirmado = new ContenidoInfoAfirma();
					documentoFirmado
							.setContenido(requestObtenerInformacionFirma
									.getContent());
					documentoFirmado.setTipoMIME(MimeUtil
							.getMimeType(requestObtenerInformacionFirma
									.getContent()));
					infoFirma.setDocumentoFirmado(documentoFirmado);
				}
			}
			...
			if (requestObtenerInformacionFirma.isObtenerTipoFirma()) {
				...
				String tipoFirma = ObtenerInformacionFirmaUtil
						.getTipoFirma(tipoFirmaDSS) + sufijo;
			...
			throw new AfirmaException(CodigosError.COD_0011,
					CodigosError.MSJ_0011);
		} catch (DSSSignerProcessorException e) {
			throw new AfirmaException(CodigosError.COD_0009,
					CodigosError.MSJ_0009);
		} catch (AfirmaWSClientException e) {
			logger.error("Error en la llamada a @firma", e);
			throw new AfirmaException(CodigosError.COD_0010,
					CodigosError.MSJ_0010);
		} catch (AfirmaException e) {
			throw e;
		} catch (Throwable t) {
			logger.error(
					"Error inesperado al intentar obtener información de la firma, ",
					t);
			throw new AfirmaException(CodigosError.COD_0000,
					CodigosError.MSJ_0000);
		}
	...
	public ResultadoAmpliarFirmaAfirma ampliarFirma(
			RequestAmpliarFirmaDSS requestAmpliarFirmaDSS)
			throws AfirmaException {
		...
		try {
			...
			// devolvemos un error //
			if (esPADES(requestAmpliarFirmaDSS.getSign())
					&& AfirmaConstantes.UPGRADE_TIMESTAMP
							.equals(requestAmpliarFirmaDSS.getConfiguracion()
									.getFormatoAmpliacion())) {
				requestAmpliarFirmaDSS.getConfiguracion().setFormatoAmpliacion(AfirmaConstantes.UPGRADE_TIMESTAMP_PDF);
			}

			// Obtenemos el objeto que construir� la petici�n a enviar a los
			// WS de Afirma.
			DSSSignerProcessor processor = dssProcessorFactory
					.getDSSSignerProcessor(requestAmpliarFirmaDSS.getSign());
			if (processor == null) {
				throw new AfirmaException(CodigosError.COD_0001,
						CodigosError.MSJ_0001);
			} else if (processor instanceof XMLEnvelopingDSSSignerProcessor) {
			...
			if (verifyResponse
					...
					&& verifyResponse
							...
							.getResultMinor()
							.contentEquals(
									DSSResultConstantes.DSS_MINOR_INCOMPLETEUPGRADEOP)) {
				...
				// correcto
					...
					&& verifyResponse
							...
							.getResultMinor()
							.contentEquals(
									DSSResultConstantes.DSS_MINOR_INCORRECTRETURNUPDATEDSIGNTYPE)) {
				logger.info("Los servicios de Afirma responden IncompleteReturnUpdatedSignatureType para operaci�n: "
						+ requestAmpliarFirmaDSS.getConfiguracion()
								.getFormatoAmpliacion());
				throw new AfirmaException(CodigosError.COD_0006,
						CodigosError.MSJ_0006);
				...
				// Comprobamos si el tipo de amplicaci�n ha sido introducido
					...
					&& verifyResponse
							...
							.getResultMinor()
							.contentEquals(
									DSSResultConstantes.DSS_MINOR_UPDATEDSIGNTYPENOTPROVIDED)) {
				logger.info("Los servicios de Afirma responden UpdatedSignatureTypeNotProvided para operaci�n: "
						+ requestAmpliarFirmaDSS.getConfiguracion()
								.getFormatoAmpliacion());
				throw new AfirmaException(CodigosError.COD_0007,
						CodigosError.MSJ_0007);
				...
				// lanzamos una excepci�n"
					...
					&& !verifyResponse
							...
							.getResultMajor()
							.contentEquals(
									DSSResultConstantes.DSS_MAJOR_PENDING)
					&& !verifyResponse
							.getResult()
							.getResultMajor()
							.contentEquals(
			...
			byte[] firmaAmpliada = processor
					.getUpgradedSignature(verifyResponse);
	...
	public void setDssProcessorFactory(
			DSSSignerProcessorFactory dssProcessorFactory) {
	...
	public String obtenerTipoFirmaDss(
			RequestObtenerInformacionFirma requestObtenerInformacionFirma)
			throws AfirmaException {
		String tipoFirmaDSS = "";
