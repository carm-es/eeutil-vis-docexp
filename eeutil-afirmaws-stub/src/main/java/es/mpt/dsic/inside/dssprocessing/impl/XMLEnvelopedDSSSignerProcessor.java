...
public class XMLEnvelopedDSSSignerProcessor extends XMLDSSSignerProcessor {
	...
	@Override
	public ContenidoFirmado getSignedData(ResponseBaseType verifyResponse,	byte[] sign) throws ContentNotExtractedException 
	{
		...
		try 
		{
			// Obtenemos la estructura DOM del XML de la firma para obtener el valor del atributo id del nodo que se ha firmado. 
...
//			se comenta esto porque aun no saca correctamente el contenido firmado, pero deberia hacerse asi eliminando el nodo signature que lleva dentro			
...
//			String valorAtributoId = lista.get(0).replace('#',' ').trim();
			...
			if(valorAtributoId!=null)
			{
				...
				// Nos quedamos con el SignedDataRefType que tenga el elemnto xpath con el valor del atributoId del nodo Firmado
			...
			//si es la raiz o el atributoId nodo firmado es nulo esto es que es todo el xml asi que eliminamos el nodo signature y lo que queda es lo firmado
			if("/".equals(xpathExpr) || valorAtributoId==null)
			{
				...
				// Convertimos el documento a array bytes.Quiere decir que firma todo el xml desde el nodo root y eliminamos el signature y devolvemos el xml entero
			...
			else
			{
				...
				String encoding = getEncodingDatosFirmados (nodoContenido, signedDataRef, doc.getInputEncoding());
			
				// Obtenemos el contenido de ese nodo.
				// Si es hash
				if (XMLUtil.isHashMimeType(nodoContenido)) {
					//contenidoFirmado.setHash(MiscUtil.decodeByEncoding(nodoContenido.getTextContent(), signedDataRefConEncodingOrFirst.getEncoding()));				
					contenidoFirmado.setHash(MiscUtil.decodeByEncoding(nodoContenido.getTextContent(), encoding));
				...
				// Si no es hash
					...
					if (XMLUtil.isXMLMimeType(nodoContenido)) {
						...
						//byte[] bytesDocumento = XMLUtil.getBytesFromNode(nodoContenido.getFirstChild(), signedDataRefConEncodingOrFirst.getEncoding()).toByteArray();					
						byte[] bytesDocumento = XMLUtil.getBytesFromNode(nodoContenido.getFirstChild(), encoding).toByteArray();
						contenidoFirmado.setBytesDocumento(bytesDocumento);
						contenidoFirmado.setMimeDocumento("application/xml");
					// Si no lo es
					} else {
						//byte[] bytesDocumento = MiscUtil.decodeByEncoding(nodoContenido.getTextContent(), signedDataRefConEncodingOrFirst.getEncoding());
						byte[] bytesDocumento = MiscUtil.decodeByEncoding(nodoContenido.getTextContent(), encoding);
						contenidoFirmado.setBytesDocumento(bytesDocumento);
	...
	/**
	 * Obtenemos el encoding del contenido firmado. En primer lugar obtenemso el atributo del nodo signedDataRef de la respuesta de
	 * afirma, si no tiene miraremos si el nodo contenido tien este atributo.
	 * @param nodoContenido
	 * @param signedDataRef
	 * @return
	 */
	private String getEncodingDatosFirmados (Node nodoContenido, SignedDataRefType signedDataRef, String encod) {
