...
public class AfirmaServiceImpl implements AfirmaService {
	...
	@Override
	public ResultadoValidacionInfoAfirma validarFirma(String aplicacion,
			String firmaElectronica, String datos, String hash,
			String algoritmo, String tipoFirma) {
		RequestValidarFirma requestValidarFirma = new RequestValidarFirma(
				checkSecurity(aplicacion), firmaElectronica, datos, hash,
				algoritmo, tipoFirma);
	...
	public ResultadoValidarCertificadoAfirma validarCertificado(
			String aplicacion, String certificado, Boolean infAmpliada) {
	...
	@Override
	public InformacionFirmaAfirma obtenerInformacionFirma(String aplicacion,
			byte[] firma, boolean obtenerFirmantes,
			boolean obtenerDatosFirmados, boolean obtenerTipoFirma,
			byte[] content) throws AfirmaException {
		RequestObtenerInformacionFirma requestObtenerInformacionFirma = new RequestObtenerInformacionFirma(
				checkSecurity(aplicacion), firma, obtenerFirmantes,
				obtenerDatosFirmados, obtenerTipoFirma, content);
//		return afirmaClient
//				.obtenerInformacionFirma(requestObtenerInformacionFirma);
		InformacionFirmaAfirma informacionFirma = afirmaClient.obtenerInformacionFirma(requestObtenerInformacionFirma);
	...
	@Override
	public ResultadoAmpliarFirmaAfirma ampliarFirma(String aplicacion,
			byte[] sign, ConfiguracionAmpliarFirmaAfirma configuracion)
			throws AfirmaException {
		RequestAmpliarFirmaDSS requestAmpliarFirmaDSS = new RequestAmpliarFirmaDSS(
				checkSecurity(aplicacion), sign, null, configuracion);
		return afirmaClient.ampliarFirma(requestAmpliarFirmaDSS);
	}

	@Override
	public String obtenerTipoFirmaDss(String aplicacion, byte[] firma,
			byte[] content) throws AfirmaException {
		RequestObtenerInformacionFirma requestObtenerInformacionFirma = new RequestObtenerInformacionFirma(
				checkSecurity(aplicacion), firma, false, false, false, content);
	...
	public ResultadoValidacionFirmaFormatoAAfirma validarFirmaFormatoA(
			String aplicacion, String firmaElectronica, String datos,
