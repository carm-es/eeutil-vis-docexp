			...
			try {
				...
				if (ObtenerInformacionFirmaUtil.isXMLDSigSignature (doc, xpath)) {					
					if (ObtenerInformacionFirmaUtil.isXMLDSigEnveloping (doc, xpath)) {
						...
						logger.debug("El documento es XMLDSigEnveloped por lo que se devuelve un XMLEnvelopingDSSSignerProcessor");
						processor = new XMLEnvelopedDSSSignerProcessor ();						
					} else {
						logger.debug("El documento no es ni XMLDSigEnveloping ni XMLDSigEnveloped, se presupone que es XMLDSigDetached, por lo que se devuelve un XMLDetachedDSSSignerProcessor");
