...
public class TemplateConverter {
  ...
  protected static final Log logger = LogFactory.getLog(TemplateConverter.class);
  ...
  public DocumentoContenido convertDocumentToPdf(DocumentoEniConMAdicionales docEniAdicionales,
      byte[] jrxml, String idApp) throws EeutilException {
    DocumentoContenido retorno = new DocumentoContenido();

    try (ByteArrayInputStream bArrayIn = new ByteArrayInputStream(jrxml)) {

      JasperReport report = JasperCompileManager.compileReport(bArrayIn);
      ...
      try (ByteArrayOutputStream bout = new ByteArrayOutputStream();
        ...
        byte[] contenidoConvertido = convertContent(
            docEniAdicionales.getDocumento().getContenido().getValorBinario(), idApp);

        PDFMergerUtility merger = new PDFMergerUtility();
        merger.setDestinationStream(pdfSalida);

        try (ByteArrayInputStream bArrayIn2 = new ByteArrayInputStream(bout.toByteArray());
            ByteArrayInputStream bArrayIn3 = new ByteArrayInputStream(contenidoConvertido)) {
          merger.addSource(bArrayIn2);
          merger.addSource(bArrayIn3);
          merger.mergeDocuments(MemoryUsageSetting.setupTempFileOnly());
        }

        // retorno.setBytesDocumento(Arrays.copyOf(pdfSalida.toByteArray(),pdfSalida.toByteArray().length));
  ...
  private byte[] convertContent(byte[] contenido, String idApp) throws EeutilException {
    byte[] retorno = null;
    File converted = null;
    try {
      ...
      converted = pdfConverter.convertir(ipOO, portOO, fileIn, null, idApp,
      ...
      // logger.error("Fallo al convertir",e);
      throw new EeutilException("Fallo al convertir" + e.getMessage(), e);
    } catch (Exception e) {
      // logger.error("Fallo al convertir",e);
      throw new EeutilException("Fallo al convertir" + e.getMessage(), e);
    }
      ...
      try {
        ...
        throw new EeutilException("Error al eliminar fichero temporal:"
            + converted.getAbsolutePath() + " " + e.getMessage(), e);
      }
    }
   ...
   * @throws IOException
   */
  private File createFile(byte[] content) throws EeutilException {
