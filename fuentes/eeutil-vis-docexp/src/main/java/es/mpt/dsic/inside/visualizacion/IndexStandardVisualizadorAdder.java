...
public class IndexStandardVisualizadorAdder {
  ...
  protected static final Log logger = LogFactory.getLog(IndexStandardVisualizadorAdder.class);
  ...
  public static Color DEFAULT_BACKGROUND_TITLE = new Color(200, 200, 200);
  ...
  protected void printTitleIndex(Document doc) throws EeutilException {
    try {
  ...
  protected void printTitle(Document doc, String title) throws EeutilException {
    ...
    try {
      ...
      throw new EeutilException("No se puede imprimir el titulo  " + title + ", " + e.getMessage(),
          e);
    }
  }
  ...
  public void addVerticalContent(List<VisualizacionItem> listaItems, Document doc)
      throws EeutilException {
    try {
      doc.newPage();
      if (title != null) {
      ...
      printMoreData(listaItems, doc);

    } catch (EeutilException e) {

      throw new EeutilException("No se puede anadir el contenido " + e.getMessage(), e);
    }
   ...
   */
  protected void printIndex(List<VisualizacionItem> listaItems, Document doc)
      throws EeutilException {
    ...
    try {
      ...
      // Contenido
      // Recorremos la lista de Items, ignoramos el primero porque ya lo hemos pintado en el
      // pre�mbulo
      ...
      for (int i = 1; i < listaItems.size(); i++) {
        ...
        tableContenido.addCell(createCeldaDescripcionContenido(
            visItem.marcadoresToString(true) + " - " + visItem.getNombre(),
            visItem.getProfundidad(), font, Element.ALIGN_LEFT));
  ...
  protected void printMoreData(List<VisualizacionItem> listaItems, Document doc)
      throws EeutilException {
  ...
  private void printPreambulo(VisualizacionItem item, Document doc) throws EeutilException {
    ...
    try {
      if (item.getPropiedades() != null) {
        ...
        for (Entry<String, String> propiedad : item.getPropiedades()) {
          ...
          celdaClave.setBorder(Rectangle.NO_BORDER);
