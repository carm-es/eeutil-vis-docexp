...
public class UnidadOrganicaController extends QuartzJobBean {
	...
	protected static final Log logger = LogFactory
			.getLog(UnidadOrganicaController.class);
	...
	protected void executeInternal(JobExecutionContext context)
		...
		try {
			...
			UnidadOrganicaController controller = (UnidadOrganicaController) appCtx
					.getBean("unidadOrganicaController");
	...
	public void loadOficinas() {
		try 
		{
			...
			if (consumerOficinas.configure()) {
				...
				// obtener fecha ultima sincronizacion. la anulamos porque esta es la fecha de creacion de registro de unidadorganica
				Date unidadOrganicaSyncDate = unidadOrganicaService.geLastSync();
				//Date unidadOrganicaSyncDate = null;
	
				String base64 = "";
				String file = rutaTemp + pathDescompresionOficinas	+ Constantes.FILE_SEPARATOR + "Oficinas.xml";
				if (unidadOrganicaSyncDate == null) 
				{
	...
	private void inserData(String file, String base64) throws IOException,
			UnidadOrganicaException {
		// borrado de datos anteriores
		Utils.deleteBefore(rutaTemp, ficheroTxtTemporal, ficheroZipTemporal,
				pathDescompresion);
		...
		Utils.getUnZippedFile(
				new FileInputStream(rutaTemp + ficheroTxtTemporal), rutaTemp
						+ ficheroZipTemporal);

		// descomprimir fichero zip
		Utils.unZipFile(rutaTemp + ficheroZipTemporal, rutaTemp
				+ pathDescompresion);
	...
	private void inserDataOficinas(String file, String base64) throws IOException, UnidadOrganicaException 
	{
		...
		// convertir fichero txt base64 a zip
		Utils.getUnZippedFile(
				new FileInputStream(rutaTemp + ficheroOficinasTxtTemporal), rutaTemp
		...
		//se hace un handleroficinas igual que el handlerunidadesorganicas y simulando un organismo pero metiendo los datos de oficina para
		//poder cargarlo en la misma tabla porque solo queremos para validar, es decir, saber que existe
	...
	public void setUnidadOrganicaService(
			UnidadOrganicaServiceImpl unidadOrganicaService) {
