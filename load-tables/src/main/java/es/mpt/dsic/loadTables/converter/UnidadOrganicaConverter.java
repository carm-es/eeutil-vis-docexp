...
public class UnidadOrganicaConverter {
	...
	public static UnidadOrganica toEntity(Organismo datos, Date syncDate) {
		...
		retorno.setCodigoUnidadOrganica(datos.getDatosIdentificativos().getUnidadOrganica().getCodigo());
		retorno.setNombreUnidadOrganica(datos.getDatosIdentificativos().getUnidadOrganica().getDescripcion());
		retorno.setNivelAdministracion(datos.getDatosIdentificativos().getNivelAdministracion());
		retorno.setEntidadDerechoPublico(datos.getDatosIdentificativos().getIndicadorEntidadDerechoPublico());
		retorno.setCodigoExterno(datos.getDatosIdentificativos().getCodigoExterno());
		
		//datos dependencia
		retorno.setCodigoUnidadSuperior(datos.getDatosDependencia().getUnidadSuperiorJerarquica().getCodigo());
		retorno.setNombreUnidadSuperior(datos.getDatosDependencia().getUnidadSuperiorJerarquica().getDescripcion());
		retorno.setCodigoUnidadRaiz(datos.getDatosDependencia().getUnidadOrganicaRaiz().getCodigo());
		retorno.setNombreUnidadRaiz(datos.getDatosDependencia().getUnidadOrganicaRaiz().getDescripcion());
		retorno.setCodigoRaizDerechoPublico(datos.getDatosDependencia().getUnidadRaizEntidadDerechoPublico().getCodigo());
		retorno.setNombreRaizDerechoPublico(datos.getDatosDependencia().getUnidadRaizEntidadDerechoPublico().getDescripcion());
