...
public class UnidadOrganicaServiceImpl extends
		GenericServiceImpl<UnidadOrganica> {

	protected static final Log log = LogFactory
			.getLog(UnidadOrganicaServiceImpl.class);

	public void saveList(List<Organismo> organimos, Date syncDate) {
		log.debug("Inicio saveList");
		if (organimos != null) {
			List<UnidadOrganica> datos = UnidadOrganicaConverter.toEntities(
					organimos, syncDate);
	...
	public void saveOrUpdateListFlush(List<UnidadOrganica> list, int countFlush) {
		...
		try {
			...
			for (UnidadOrganica data : list) {
				try {
					objectAux = getGenericDao().findById(
							getClavePrimaria(data), session);
					if (objectAux != null) {
					...
					log.debug("Error al procesar el registro: "
							+ data.toString());
				}
			}
			tx.commit();
