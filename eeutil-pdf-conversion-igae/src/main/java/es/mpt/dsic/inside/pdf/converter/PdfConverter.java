...
public class PdfConverter {
	 ...
	 * Convierte a PDF
	 */
	public File convertir(String ipOO, String portOO, File contenido,
			String mimeType) throws PdfConversionException {
		...
		try {
			if (mime == null) {
				...
				mime = MimeUtil.getMimeNotNull(IOUtil
						.getBytesFromObject(contenido));
				pdf = convertir(ipOO, portOO, contenido, mime);

			} 
			else if (mime.contentEquals("application/vnd.openxmlformats-officedocument.wordprocessingml.document")) {
				...
				|| mime.contentEquals("text/tcn")) {

				pdf = tcnToPdfConverter.convertTCNToPdf(IOUtil
						.getBytesFromObject(contenido));

			}else if (StringUtil.contiene(mime, "text")
					|| StringUtil.contiene(mime, "doc")
					|| StringUtil.contiene(mime, "ppt")
					|| StringUtil.contiene(mime, "ms")
					|| StringUtil.contiene(mime, "msword")
					|| StringUtil.contiene(mime, "xls")
					|| StringUtil.contiene(mime, "rtf")
					|| StringUtil.contiene(mime, "wpd")) {

				if (StringUtil.contiene(mime, "xml"))
					mime = "text/plain";

				pdf = officeToPdfConverter.convertOfficeToPDFByMIME(contenido,
						mime, ipOO, portOO);
			...
			} else if (StringUtil.contiene(mime, "tiff")
					|| StringUtil.contiene(mime, "jpg")
					|| StringUtil.contiene(mime, "BMP")
					|| StringUtil.contiene(mime, "image")
					|| StringUtil.contiene(mime, "jpeg")
					|| StringUtil.contiene(mime, "tif")
					|| StringUtil.contiene(mime, "gif")
					|| StringUtil.contiene(mime, "bmp")
					|| StringUtil.contiene(mime, "gif")
				...
				if (StringUtil.contiene(mime, "tiff") || StringUtil.contiene(mime, "tif"))
					...
					pdf = genericPdfConverter
							.convertPdfGeneric(contenido, true);
