...
public class SoapAuthenticationObjectInterceptor extends
		AbstractPhaseInterceptor<Message> {

	protected final static Log logger = LogFactory
			.getLog(SoapAuthenticationObjectInterceptor.class);
	 ...
	 * 
	 * boolean existe=false; for(Object obj:inObjects) { if(obj instanceof
	 * ApplicationLogin) { ApplicationLogin info=(ApplicationLogin)obj;
	 * 
	 * authenticationManager.authenticate(new
	 * UsernamePasswordAuthenticationToken( info.getIdApplicacion(),
	 * info.getPassword()));
	 * 
	 * message.getExchange().put(Authentication.class, new
	 * UsernamePasswordAuthenticationToken( info.getIdApplicacion(),
	 * info.getPassword())); existe=true; } } if(!existe) {
	 * message.getExchange().put(Authentication.class, new
	...
	public void handleMessage(Message message) throws Fault {
		...
		AplicacionOperacionObject appOperacion = administrationService
				.incrementarPeticionAccion(user,
						StringUtils.capitalize(qNampeSoap.getLocalPart()));

		byte[] xmlRequest = (byte[]) message.get("xmlRequest");
		if (appOperacion.isCapturar()
			&& xmlRequest != null) {
	...
	private void capturarPeticion(AplicacionOperacionObject appOperacion,
			byte[] data) {
		PeticionObject peticion = new PeticionObject(
				appOperacion.getAplicacion(), appOperacion.getOperacion(),
				new Date(), data);
