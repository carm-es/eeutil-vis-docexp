...
public class EeutilAdministrationService implements AdministrationService {
	...
	protected final static Log logger = LogFactory
			.getLog(EeutilAdministrationService.class);

	@Autowired
	private EeutilDao eeutilDao;

	@Override
	public AplicacionOperacionObject incrementarPeticionAccion(
			String idAplicacion, String operacion) {
		...
		List<EeutilAplicacionOperacion> eeutilAppList = eeutilDao.findObjetosWithOrder(EeutilAplicacionOperacion.class, criterios, orden);
		...
		return new AplicacionOperacionObject(databaseObject.getIdAplicacion(),
				databaseObject.getOperacion(),
				databaseObject.getNumeroPeticiones(),
				databaseObject.isCapturar());
	}

	private List<Criterion> criteriasPkPeticionAplicacion(String idAplicacion,
			String operacion) {
	...
	@Override
	public void modificarAplicacionOperacion(
			AplicacionOperacionObject aplicacionOperacion) {
		EeutilAplicacionOperacion databaseObject = (EeutilAplicacionOperacion) eeutilDao
				.findObjeto(
						EeutilAplicacionOperacion.class,
						criteriasPkPeticionAplicacion(
								aplicacionOperacion.getAplicacion(),
	...
	public void eliminarTodasPeticiones() {
		List<EeutilPeticion> peticiones = eeutilDao
				.getAllObjetos(EeutilPeticion.class);
	...
	public void eliminarPeticion(PeticionObject peticion) {
		List<EeutilPeticion> databaseObjects = (List<EeutilPeticion>) eeutilDao
				.findObjetos(
						EeutilPeticion.class,
						criteriasPkPeticion(peticion.getAplicacion(),
	...
	private List<Criterion> criteriasPkPeticion(String idAplicacion,
			String operacion) {
		List<Criterion> retorno = new ArrayList<Criterion>();
	...
	public List<PeticionObject> consultarPeticiones(PeticionObject peticion) {
		...
		List<EeutilPeticion> databaseObjects = (List<EeutilPeticion>) eeutilDao
				.findObjetosWithOrder(
						EeutilPeticion.class,
						criteriasPkPeticion(peticion.getAplicacion(),
								peticion.getOperacion()),
						orden);
		if (CollectionUtils.isNotEmpty(databaseObjects)) {
			retorno.addAll(ConvertDataEeutil
					.converDataPeticion(databaseObjects));
		}
		return retorno;
	}
	...
	public byte[] descargarPeticion(PeticionObject peticion) {
		List<Criterion> criterios = criteriasPkPeticion(
				peticion.getAplicacion(), peticion.getOperacion());
		criterios.add(Restrictions.eq("fecha", peticion.getFecha()));

		EeutilPeticion databaseObject = (EeutilPeticion) eeutilDao.findObjeto(
				EeutilPeticion.class, criterios);
