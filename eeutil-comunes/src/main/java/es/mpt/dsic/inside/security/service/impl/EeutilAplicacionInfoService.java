...
public class EeutilAplicacionInfoService implements AplicacionInfoService {
	...
	protected final static Log logger = LogFactory
			.getLog(EeutilAplicacionInfoService.class);

	@Autowired
	private EeutilDao eeutilDao;

	private final String DELETE_PLANTILLAS_BY_APLICACION = "DELETE FROM EeutilAplicacionPlantilla where id.idaplicacion = ?";

	@Override
	public AppInfo getAplicacionInfo(String appId)
			throws AuthenticationException, DataAccessException {

		EeutilAplicacion isApp = (EeutilAplicacion) eeutilDao.getObjeto(
				EeutilAplicacion.class, appId);
	...
	public UserDetails loadUserByUsername(String username)
		...
		EeutilAplicacion isApp = (EeutilAplicacion) eeutilDao.getObjeto(
				EeutilAplicacion.class, username);

		if (isApp == null) {
			throw new UsernameNotFoundException(
					"No se puede encontrar la aplicación: " + username,
					username);
		}
		...
		EeutilUserDetails user = new EeutilUserDetails(username,
				isApp.getPassword(), isApp.isActiva(), true, true, true,
				permisos, new EeutilAppInfo(isApp));
		return user;
	}

	@Override
	public AppInfo asociarPantilla(String appId, String idPlantilla,
			byte[] plantilla) {
		...
		if (CollectionUtils.isNotEmpty(appEntity.getPlantillas())) {
			for (EeutilAplicacionPlantilla plantillaEntity : appEntity
					.getPlantillas()) {
				if (plantillaEntity.getId().getIdplantilla()
						.equals(idPlantilla)) {
		...
		if (newPlantilla) {
			...
			appEntity
					.getPlantillas()
					.add(new EeutilAplicacionPlantilla(
							new EeutilAplicacionPlantillaId(appId, idPlantilla),
							plantilla));
		}
		...
		if (CollectionUtils.isNotEmpty(appEntity.getPlantillas())) {
			...
			CollectionUtils.filter(appEntity.getPlantillas(), new Predicate() {
				...
				public boolean evaluate(Object object) {
					return !((EeutilAplicacionPlantilla) object).getId()
							.getIdplantilla().equals(idPlantilla);
				}
			});
